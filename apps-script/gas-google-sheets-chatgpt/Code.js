...
function getApiKey_() {
    if (CACHE_API_KEY)
        return CACHE_API_KEY; // ① memory cache
    const props = PropertiesService.getScriptProperties();
    let key = (props.getProperty(PROP_OPENAI_KEY) || '').trim();
    if (key)
        return (CACHE_API_KEY = key); // ② property exists
    // ③ prompt only when property is missing
    const ui = SpreadsheetApp.getUi();
    const res = ui.prompt('設定 OpenAI API 金鑰', '請貼上您的 OpenAI API Key（僅儲存於此 Apps Script）：', ui.ButtonSet.OK_CANCEL);
...
function extractRespText(body) {
    for (const item of body.output || []) {
        if (item.type !== 'message')
            continue;
        const txtObj = (item.content || []).find((c) => c.type === 'output_text' && typeof c.text === 'string');
        if (txtObj?.text)
            return txtObj.text.trim();
    }
    return '';
}
/**
 ...
 */
function ChatGPT(prompt, temperature = TEMPERATURE, max_tokens = MAX_TOKENS, model = MODEL) {
    const payload = {
        model,
        input: [
            {
 ...
 */
function WebSearch(query, contextSize = 'high', temperature = TEMPERATURE, max_tokens = MAX_TOKENS, model = MODEL) {
    const payload = {
        model,
        input: [
            {
 ...
 */
function OpenAIVision(imageUrl, prompt = DEFAULT_VISION_PROMPT, max_tokens = 300, model = 'gpt-4o', temperature = 0) {
    if (!imageUrl)
        throw new Error('需要提供圖片網址 (imageUrl)。');
    const payload = {
        model,
        input: [
            {
 ...
 */
function OpenAITTS(text, voice = DEFAULT_VOICE, model = DEFAULT_SPEECH_MODEL, instructions = DEFAULT_SPEECH_INSTRUCTIONS, responseFormat = DEFAULT_SPEECH_FMT, filename = '') {
    if (!text)
        throw new Error('Text 不可為空白！');
    const payload = {
        model,
        input: text,
        voice,
    ...
    if (instrTrim && !/^tts-1(-hd)?$/i.test(model))
        payload.instructions = instrTrim;
    const options = {
        method: 'post',
        contentType: 'application/json',
        headers: { Authorization: `Bearer ${getApiKey_()}` },
    ...
    const code = res.getResponseCode();
    if (code !== 200)
        throw new Error(`OpenAI TTS error (${code}): ${res.getContentText()}`);
 ...
 * @customfunction
 */
function gpt_image(promptRef, size = '1024x1024', quality = 'high') {
    return 'Ready for Run OpenAI';
}
/**
 ...
 * @customfunction
 */
function gpt_vision(imageUrlRef, promptRef = `"${DEFAULT_VISION_PROMPT}"`, max_tokens = 300, model = 'gpt-4o', outputDirection = 'right') {
    return 'Ready for Run OpenAI';
}
/**
 ...
 * @customfunction
 */
function gpt_speech(textRef, voice = `"${DEFAULT_VOICE}"`, model = `"${DEFAULT_SPEECH_MODEL}"`, instructions = `"${DEFAULT_SPEECH_INSTRUCTIONS}"`, responseFormat = `"${DEFAULT_SPEECH_FMT}"`, filename = '""', outputDirection = 'right') {
    return 'Ready for Run OpenAI';
}
// ────────────────────────────────
        ...
        for (let c = 1; c <= cols; c++) {
            ...
            if (formula.includes('gpt_text(')) {
                ...
                try {
                    ...
                    if (looksLegacy) {
                        ...
                        const direction = (resolveConcat(dirRaw).replace(/"/g, '').toLowerCase() ||
                            'right');
                        const isParallel = /true|1|parallel/i.test(resolveConcat(args[5] || 'false').replace(/"/g, ''));
                        const outCell = direction === 'below'
                            ? sheet.getRange(cell.getRow() + 1, cell.getColumn())
                            : sheet.getRange(cell.getRow(), cell.getColumn() + 1);
                        if (isParallel) {
                    ...
                    fallbackOutCell.setValue(`ERROR: ${err.message}`);
                }
                continue;
            }
            /* ---------- GPT_SEARCH ---------- */
            ...
            if (formula.includes('gpt_vision(')) {
                try {
                    ...
                    const [urlRaw, promptRaw = `"${DEFAULT_VISION_PROMPT}"`, tokensRaw = 300, modelRaw = '"gpt-4o"', dirRaw = '"right"',] = argStr.split(/\s*,\s*/);
            ...
            if (formula.includes('gpt_speech(')) {
                try {
                    ...
                    const [textRaw, voiceRaw = `"${DEFAULT_VOICE}"`, modelRaw = `"${DEFAULT_SPEECH_MODEL}"`, instrRaw = `"${DEFAULT_SPEECH_INSTRUCTIONS}"`, fmtRaw = `"${DEFAULT_SPEECH_FMT}"`, fileRaw = '""', dirRaw = '"right"',] = argStr.split(/\s*,\s*/);
